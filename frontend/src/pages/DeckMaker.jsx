...
const DeckMaker = ({seriesData, cardSeries}) => {
    ...
    const sortCards = (cards) => {
        return [...cards].sort((a, b) => {
            if (a.category !== b.category) {
                // Pokemon(P)과 Trainer(T)를 비교하여 Pokemon이 먼저 오도록 설정
                return a.category === "Pokemon" ? -1 : 1;
            }
            // 카테고리가 같으면 ID 순 정렬
    ...
    const handleCardClick = async (card) => {
        ...
        try {
            // 상세 데이터가 이미 있는지 확인하고 없으면 fetch (에너지 타입)
            ...
            setSelectedCards(prev => sortCards([...prev, detailedData]));
            ...
            setSelectedCards(prev => sortCards([...prev, card]));
        }
    }
                                ...
                                <ul>
                                    {visibleCards.map((card) => (
                                        <li key={card.id} onClick={() => {
                                            ...
                                            <img src={`${card.image}/low.webp`} alt={card.name} onError={handleImageError} loading="lazy" />
                                            <p>{card.name}</p>
                                        </li>
                                    ))}
                                </ul>
