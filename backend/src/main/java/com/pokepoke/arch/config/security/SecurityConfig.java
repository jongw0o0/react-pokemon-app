package com.pokepoke.arch.config.security;
...
@EnableWebSecurity  // 스프링 시큐리티를 활성화 하는 어노테이션
public class SecurityConfig {
    ...
    SecurityFilterChain filterChain(HttpSecurity http, CustomOAuth2UserService customOAuth2UserService) throws Exception {
        http
                ...
                // CORS 설정
                // 리액트와 부트의 포트번호가 다르므로 포트 번호를 명시적으로 설정
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers(HttpMethod.PUT, "/api/decks/**").authenticated()
    ...
    // formLogin을 사용할 때는 시큐리티가 인증을 자동으로 설정하여 호출하는데
